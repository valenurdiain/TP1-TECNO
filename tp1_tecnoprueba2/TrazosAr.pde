class TrazosAr {
  ...
  void velocidadMaxima(float pitch) {
    // variacionVelocidad = pitch * VELOCIDAD_VARIACION;
    
   variacionVelocidad = pitch * Di.vel;
   
   Di.posY += Di.vel = Di.vel *+ 10; 
  println ("velocidad" + Di.posY);
}
  
  void velocidadMinima(float pitch) {
     Di.posY -= Di.vel = Di.vel *- 10;
  }
}
